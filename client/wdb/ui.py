...
class Interaction(object):
    ...
    def do_select(self, data):
        self.index = int(data)
        self.db.send(
            "SelectCheck|%s" % dump({"frame": self.current, "name": self.current_file})
        )
    ...
    def do_file(self, data):
        ...
        self.db.send(
            "Select|%s" % dump({"frame": self.current, "name": fn, "file": file})
        )
    ...
    def do_eval(self, data):
                        ...
                        if self.db._importmagic_index:
                            ...
                            for _, module, variable in scores:
                                if variable is None:
                                    ...
                                    imports.append(
                                        "from %s import %s" % (module, variable)
                                    )
        ...
        if redir and not self.db.hooked:
            ...
            self.db.send(
                "Print|%s"
                % dump({"for": raw_data, "result": result, "duration": duration})
            )
            if imports:
    ...
    def do_breakpoints(self, data):
        self.db.send(
            "Print|%s" % dump({"for": "Breakpoints", "result": self.db.breakpoints})
        )
    ...
    def do_save(self, data):
        ...
        try:
            ...
            self.db.send(
                "Echo|%s" % dump({"for": "Save succesful", "val": "Wrote %s" % fn})
            )
    ...
    def do_diff(self, data):
        ...
        expressions = [
            expression.strip()
            for expression in (data.split("?") if "?" in data else data.split("<>"))
        ]
        strings = []
